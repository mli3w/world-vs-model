...
def _evolution_html(fundamental, pred_path=PREDICTIONS, results_path=RESULTS_PATH):
    ...
    return (head
            ...
            '<p class="note sub">Surprise is measured in <b>bits</b> — basically, '
            '<i>how many coin flips this is worth</i>. If someone guesses a coin and gets it right, '
            'that\'s <b>1 bit</b> of luck. Getting two in a row is <b>2 bits</b> (1-in-4). Three in a '
            'row is <b>3 bits</b> (1-in-8). So a <b>3.5-bit</b> result is roughly like calling a coin '
            'right three-and-a-half times in a row — that\'s how shocking the prior thought it was. '
            'Each upset shows the prior used: '
